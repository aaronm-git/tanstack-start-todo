# Developer Documentation (Implementation)

Docs for contributors: how features are implemented and where to change things.

## Architecture

- [Architecture overview](./architecture-overview.md)
- [Routing (file-based)](./routing.md)
- [Authentication (Better Auth)](./authentication.md)
- [AI Todo Creation (OpenAI + TanStack AI)](./ai-todo-openai.md)
- [Zod 4 + TanStack AI Integration](./zod-tanstack-ai-integration.md)

## Type System

- [TypeScript Type System](./typescript-types.md)
- [TypeScript Best Practices](./typescript-best-practices.md)
- [Zod Schema Design Best Practices](./zod-tanstack-ai-integration.md#schema-design-best-practices)

